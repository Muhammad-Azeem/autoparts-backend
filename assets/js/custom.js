...
function sendData(url,obj){
    return $.ajax({
        ...
        "success" : function(data){
            ...
            $('#msg').html("<div class=\"container mt-3 mb-3\">\n" +
                "            <div class=\"alert alert-success alert-block\">\n" +
                "                <button type=\"button\" class=\"close\" data-dismiss=\"alert\">×</button>\n" +
                "                <strong>Status successfully updated</strong>\n" +
                "            </div>\n" +
                "        </div>").fadeIn('slow');
        }
    });
...
$(document).ready(function(){
    ...
    $("._userDetail").click(function(){
        ...
        d = JSON.parse(d);
        console.log(d);
        $("#cstatus .name").html(d[0].name);
        $("#cstatus .email").html(d[0].email);
        $("#cstatus .date").html(d[2].create_d);
        $("#cstatus .country").html("Pakistan");
        $("#cstatus .city").html(d[1].bill_loc);
        $("#cstatus .zip").html(d[1].zip);
        $("#cstatus .phone").html(d[1].phone_1+" , "+d[1].phone_2+" , "+d[1].phone_3);
        $("#cstatus .address").html(d[1].bill_addr);

        $("#cstatus .modal-header").attr("style", d.style);
       // $("#cstatus .cname").html(d.name[1]+" ( "+d.status+" )");
        $("#cstatus").modal("show");
    });
    ...
    $(".tcount").keyup(function(){
        ...
        if (t=="text"){
            ...
            var s = v.split(',');
            $(this).closest(".input-group").find(".countShow").text(s.length);
        }
    });
    ...
    $(".-change-vostat").click(function(){
        ...
        if (is_check==false){
            ...
            //$("#cstatus").modal("show");
            sendData(products_url, data).done(function(d){
                $("#cstatus").modal("hide");
